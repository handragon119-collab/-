...
- 스레드 댕댕이 은어를 자연스럽게: 스친(스레드 친구), 첫스레드, 프로필 링크, 인스타·유튜브 놀러와
- ‼️ '스하리=반하리', '반하리', '뒷삭없는 반하리' 같은 맞팔·하트갚기 약속 멘트는 절대 쓰지 마라.
  (직접 맞팔·리포스트를 해줄 수 없어서 지키지 못할 약속이다.)
- 댓글 유도는 부담 없이: "나랑 칭구하자~", "댓글 남겨조" 정도까지만. 맞팔 약속은 X.
...
class ThreadsPipeline:
    ...
    def write_reply(self, style_extra: str, examples: list[str],
        ...
        system = STYLE_GUIDE + (style_extra or "") + (
            ...
            "‼️ '스하리=반하리', '반하리', '맞팔 갈게' 같은 맞팔·하트갚기 약속 멘트는 절대 쓰지 마라.]"
        )
        user = (
            f"[내 게시글]\n{post_text[:300]}\n\n"
            f"[누가 단 댓글]\n{comment_text[:300]}\n\n"

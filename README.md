...
아이언맨의 자비스처럼 **👏 박수를 치면 깨어나 🎸 Back in Black을 틀고, 영어로 인사한 뒤 한국어로 대화·명령을 수행**하는 파이썬 비서입니다.
...
- **👏 박수로 깨우기**: 박수 소리를 감지하면 자비스가 깨어남 (호출어 모드로도 전환 가능)
- **🎸 깨어날 때 Back in Black 재생**: AC/DC - Back in Black을 틀어줌
- **🇬🇧→🇰🇷 언어 자동 전환**: 웰컴 멘트는 **영어(영국식 남성 = 영화 자비스 톤)**, 이후 대화는 **한국어 음성**으로 자동 전환
...
- 영화 속 자비스는 **영국 남성** 음성입니다. 그래서 영어 웰컴은 시스템에 설치된 **영국식 남성 음성(en-GB)**을 자동으로 골라 씁니다.
  - 더 똑같이 만들고 싶으면 OS에 영국 음성을 추가하세요 — macOS: `시스템 설정 → 손쉬운 사용 → 음성 콘텐츠`에서 *Daniel(영국)* 추가 / Windows: *George(영국)* 설치 / Linux: `sudo apt install mbrola mbrola-en1` 또는 espeak 영국 음성.
  - **진짜 영화 수준의 목소리**는 [ElevenLabs](https://elevenlabs.io)의 자비스풍 보이스를 쓰는 게 가장 비슷합니다 (브레인을 그쪽 TTS로 교체하면 됨).
- **Back in Black 음원**: 저작권 때문에 저장소에 포함하지 않습니다.
  - 가지고 있는 mp3가 있으면 `.env`에 `JARVIS_WAKE_MUSIC=/경로/back_in_black.mp3` 지정 (또는 `assets/back_in_black.mp3`에 두기)
  - 없으면 깨어날 때 유튜브에서 자동으로 'Back in Black'을 열어줍니다.
...
**사용법**: 👏 박수를 치면 → 🎸 Back in Black이 흐르고 → 🇬🇧 *"Good day, sir. J.A.R.V.I.S. online and at your service."* → 이제 🇰🇷 한국어로 말하면 됩니다.
예) *"오늘 며칠이야?"*, *"유튜브에서 아이언맨 검색해줘"*

대화가 끝나면 잠시 조용히 있으면 다시 대기 모드로 돌아갑니다. 완전히 끄려면 **"종료"** 또는 Ctrl+C.
(텍스트 모드에서는 박수 대신 **Enter** 로 깨웁니다.)
...
| 파일 | 역할 |
|------|------|
| `jarvis.py` | 메인 루프 (박수 대기 → 음악 → 영어 웰컴 → 한국어 대화) |
| `voice.py` | 귀·입·박수감지 (STT / TTS / 언어 자동 전환 / clap 감지) |
| `music.py` | 깨어날 때 Back in Black 재생 |
